...
if (!empty($doaction)) {
  ...
  case 'downloads3': //Download S3 Backup
    ...
    $jobid=$_GET['jobid'];
    try {
      CFCredentials::set(array('backwpup' => array('key'=>$jobs[$jobid]['awsAccessKey'],'secret'=>$jobs[$jobid]['awsSecretKey'],'default_cache_config'=>'','certificate_authority'=>true),'@default' => 'backwpup'));
      $s3 = new AmazonS3();
